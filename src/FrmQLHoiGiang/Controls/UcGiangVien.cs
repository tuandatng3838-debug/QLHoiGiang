...
public partial class UcGiangVien : UserControl
{
    ...
    {
        ReloadLookupsPreserveSelection();
    }

    private void ReloadLookupsPreserveSelection()
    {
        var selectedDonVi = GetSelectedId(cboDonVi);
        var selectedTrinhDoCm = GetSelectedId(cboTrinhDoCM);
        var selectedTrinhDoLlct = GetSelectedId(cboTrinhDoLLCT);
        var selectedCapBac = GetSelectedId(cboCapBac);
        var selectedChucDanh = GetSelectedId(cboChucDanh);
        var selectedHocHam = GetSelectedId(cboHocHam);
        var selectedHocVi = GetSelectedId(cboHocVi);

        LoadLookups();

        RestoreSelection(cboDonVi, selectedDonVi);
        RestoreSelection(cboTrinhDoCM, selectedTrinhDoCm);
        RestoreSelection(cboTrinhDoLLCT, selectedTrinhDoLlct);
        RestoreSelection(cboCapBac, selectedCapBac);
        RestoreSelection(cboChucDanh, selectedChucDanh);
        RestoreSelection(cboHocHam, selectedHocHam);
        RestoreSelection(cboHocVi, selectedHocVi);
    }
    ...
    private void ApplyQuickFilter()
    {
        ...
        _binding.DataSource = _data.Where(g =>
            g.MaSo.ToLowerInvariant().Contains(needle) ||
            g.HoTen.ToLowerInvariant().Contains(needle) ||
            (g.SoDienThoai ?? string.Empty).ToLowerInvariant().Contains(needle) ||
            (g.Email ?? string.Empty).ToLowerInvariant().Contains(needle)
        ).ToList();
    }
    ...
    private void gridGiangVien_CellClick(object sender, DataGridViewCellEventArgs e)
    {
        ...
        var entity = _binding[e.RowIndex] as GiangVien;
        if (entity == null)
        {
            return;
        }
    ...
    private static void RestoreSelection(SiticoneComboBox combo, int? value)
    {
        if (value.HasValue)
        {
            combo.SelectedValue = value.Value;
    ...
    private void cboTrinhDoCM_SelectedIndexChanged(object sender, EventArgs e)
    {
